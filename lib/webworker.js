...
var Worker = function(src, opts) {
    ...
    // The 'name' of this worker. To be used for name based communication
    // and events.
    var name = undefined;
    ...
    var start = function() {
        wsSrv.addListener('listening', function() {
            ...
            var baseSrc = path.basename(src);
            self.name = opts.name || baseSrc.substring(0, baseSrc.lastIndexOf('.')) || baseSrc;
            ...
            self.remoteEmit('ready');
            self.emit('spawned', pid);
    ...
    // Trigger an event on the worker's side
    self.postEvent = function(msg, fd) {
        postMessageImpl(wwutil.MSGTYPE_EVENT, msg, fd);
    };
    
    self.remoteEmit = function(eventType) {
        var args = Array.prototype.slice.call(arguments, 1);
        args.unshift(eventType);
        self.postEvent(args);  
    };
    
    self.listen = function(obj, eventType){
...
sys.inherits(Worker, events.EventEmitter);
